//! Quadratic-funding math — pure, chain-free.
//!
...
//! whale. It touches no `Env`, no storage, no `require_auth` — just integers — so
//! every rule here is unit-testable in isolation. The chain glue (reading
//! `Contribution`/`Donors` from storage, writing `matched`) lives in `finalize` /
//! `preview_matches`, which call into these functions.
//!
//! **Overflow → `Error`:** the error set has no dedicated `Overflow` variant, and the
//! products stay ≤ ~4·10²⁰ ≪ `i128::MAX`, so overflow is unreachable in practice. Where
//! checked arithmetic could still fail we surface `Error::InvalidAmount` (the closest
//! "the numbers are out of range" variant) rather than `panic!`, keeping the failure
//! legible off-chain.

use crate::Error;

/// Integer square root: `⌊√n⌋` for any `u128`.
///
...
/// Weight of one project from its running `Σ_{d} isqrt(cumulative_d)`: the square.
///
...
/// Reference weight of one project: `( Σ_{d} isqrt(cumulative_d) )²`.
///
/// `donor_totals` holds each **distinct** donor's **cumulative** contribution to this
/// project — the sqrt is taken on the summed-per-donor total exactly once, which is what
/// stops a single donor from inflating breadth by splitting one gift into many.
...
///
/// - `total_weight == 0` (every project has 0 donors) → `Error::NothingToMatch`.
...
pub fn compute_matches(pool: i128, weights: &[u128], out: &mut [i128]) -> Result<(), Error> {
    ...
    for (i, &w) in weights.iter().enumerate() {
        // pool_u·w ≤ 10⁸·4·10¹² = 4·10²⁰ ≪ u128::MAX; checked regardless.
...
mod tests {
    ...
    fn project_weight_sums_sqrt_of_each_donor() {
        ...
        // No donors / non-positive entries → weight 0 (never panics).
    ...
    fn compute_matches_whale_vs_crowd_conserves_pool() {
        // The whale-vs-crowd shape at the weight level: same direct total, different breadth.
        ...
        // Exact pool conservation via the remainder rule.
    ...
    fn compute_matches_zero_donor_project_gets_zero() {
        // A 0-weight project receives 0 and never causes a divide-by-zero.
        let mut out = [0i128; 3];
