 ...
 * (§6.2 optimistic `donor_count` bump). The contract only exposes an aggregate
 * `donor_count`, not a per-donor lookup, so this in-memory set is the cheap signal the
 * spec allows; a page reload starts empty, which only matters if the same wallet had
 * already contributed in an earlier session — harmless for the single live demo chip-in.
 */
